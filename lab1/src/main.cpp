...
int main() {
    ...
    vertices[1] = {
    //    X      Y      Z     R     G     B
        -0.5f, -0.8f,  0.0f, 0.0f, 0.0f, 0.2f,
         0.5f, -0.9f,  0.0f, 0.0f, 0.0f, 0.2f,
    ...
    glm::vec4 ubo_color = { 0.0, 0.0, 0.0, 0.0 };
    ...
    for(size_t i = 0; i < MAX_FRAMES_IN_FLIGHT * NUM_OBJECTS; ++i) {
        ...
        descriptor_buffer_info.range = sizeof(ubo_color);
        descriptor_buffer_info.offset = 0;

        std::array<VkWriteDescriptorSet, 1> descriptor_set_writes = {};
    ...
    auto render_frame = [&](VkCommandBuffer command_buffer, uint32_t image_index, uint32_t current_frame) {
        ...
        *(glm::vec4*)uniform_buffer_mapping[current_frame * NUM_OBJECTS + 0] = ubo_color;
        ...
        vkCmdDraw(command_buffer, vertices[0].size(), 1, 0, 0);

        *(glm::vec4*)uniform_buffer_mapping[current_frame * NUM_OBJECTS + 1] = glm::vec4(0.0, 0.0, 0.0, 0.0);
        ...
        vkCmdDraw(command_buffer, vertices[1].size(), 1, 0, 0);

        imgui_new_frame();
        ImGui::ColorEdit4("clear colour", (float*)&clear_color);
        ImGui::ColorEdit4("triangle colour", (float*)&ubo_color);
